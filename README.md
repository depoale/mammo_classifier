...
• `Data` class is used to handle and manage the datasets of mammography gray-scale images to be processed: it is called to perform data augmentation and Wavelet-based filtering procedures and contains the `get_random_images method`, a useful function which returns random images from one or both classes (Label = 0 and Label = 1) of a certain dataset. <br>
...
As part of the analysis, we included the possibility to "visualise" what the model has learnt from images using GradCAM algorithm (Gradient-weighted Class Activation Mapping). Selecting the most reliable of the five model (according to the ensemble's weights), the GradCAM algorithm was employed to highlight which regions of an input image are relevant in the decision making
process. <br>
The user can choose (by setting the parameter `gradcam`) the number of images randomly extracted from the processed dataset to visualize through the GradCAM.  
